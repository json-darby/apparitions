    ...
    <div className="w-full h-screen bg-black text-white relative flex flex-col items-center justify-center overflow-hidden animate-in fade-in duration-[3000ms]">
      ...
      <nav className="fixed top-0 left-0 w-full h-[65px] md:h-[100px] z-[100] px-4 md:px-12 flex items-center justify-between pointer-events-none mix-blend-difference flex-shrink-0">
        ...
        <div className="pointer-events-auto flex items-center gap-4">
          <MobileNav current="help" title="APPARITIONS: ?" onNavigate={(scene) => onNavigate(scene)} />
          <button onClick={() => onNavigate('contact')} className="hidden sm:inline-block px-6 py-2 border border-white/20 rounded-full text-xs font-bold uppercase tracking-wider text-white hover:bg-white hover:text-black transition-colors duration-500">
            Contact
          </button>
        </div>
      </nav>
